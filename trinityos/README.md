...
1. Живой NET-мост Workbench: замена `goldenBackend` на fetch к USGS/OpenSky
   тем же интерфейсом `NetBackend` (форматы записей зафиксированы спеками).
2. WebSocket-транспорт кадров в обсервер (сейчас — файл frames.js).
3. Предложение расширения ISA: косвенная адресация + широкий immediate
   (снимет ADR-M1 и развёрнутые циклы).
4. Ядро: кооперативный планировщик + загрузчик с проверкой seal.
